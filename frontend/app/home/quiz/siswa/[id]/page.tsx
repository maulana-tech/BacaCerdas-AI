...
// Saklar ini memungkinkan Anda beralih dengan mudah ke API nanti
...
export default function ReadQuizPage() {
  ...
  // Fungsi ini sudah diimplementasikan dengan benar untuk memuat data dummy atau API
  const loadQuiz = async (id: string) => {
    ...
    try {
      ...
      if (USE_DUMMY_DATA) {
        // BENAR: Mencari kuis dari array dummy berdasarkan ID dari URL
        ...
        // BENAR: Blok ini akan berjalan jika USE_DUMMY_DATA = false
        const response = await fetch(`/api/quiz/${id}`);
        if (!response.ok) throw new Error("Kuis tidak ditemukan di API");
        const result = await response.json();
        quizData = result.data;
      }

      // BENAR: Logika ini memproses data baik dari dummy maupun API
      if (quizData) {
        setQuiz(quizData);
  ...
  // Sisa kode Anda sudah sangat baik dan tidak perlu diubah.
  // ... (SALIN SEMUA SISA KODE DARI FILE ASLI ANDA DI SINI)
  // ...
                      ...
                      {question.type === "multiple_choice" ? (
                        <div className="space-y-2">
                          {question.options?.map((option, optIndex: number) => {
                            ...
                            } else if (optIndex === userAnswer.answer && !option.is_correct) {
